...
router.get(
  ...
  (req: IRequest, _context: EventContext<{}, any, Record<string, unknown>>) => {
    return Response.redirect(
      new URL(
        `${req.params.locale ? `/${req.params.locale}` : ``}/content/category/${
          req.params.category
        }`,
        req.url
      ),
...
router.get(
  ...
  (req: IRequest, _context: EventContext<{}, any, Record<string, unknown>>) => {
    return Response.redirect(
      new URL(
        `${req.params.locale ? `/${req.params.locale}` : ``}/content/${
          req.params.slug
        }`,
        req.url
      ),
...
router.get(
  "/data/*",
  async (req, context: EventContext<{}, any, Record<string, unknown>>) => {
...
router.all(
  ...
  async (req, context: EventContext<{}, any, Record<string, unknown>>) => {
    ...
    if (pageContext.redirectTo) {
      return Response.redirect(new URL(pageContext.redirectTo, req.url), 301);
